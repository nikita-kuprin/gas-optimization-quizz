...
contract MultiSigWalletOptimized {
    ...
    function submitTransaction(address destination, uint256 value) public onlyOwner {
        uint256 tid = transactions.length;
        transactions.push(
            Transaction({
                transactionID: tid,
        ...
        notConfirmed(transactionId)
    {
        ...
        uint256 c = t.confirmationCount + 1;
        t.confirmationCount = c;
    ...
    function getConfirmations(uint256 transactionId) public view returns (address[] memory) {
        ...
        for (uint256 i; i < length; ) {
            ...
            if (confirmations[transactionId][ownerAddr]) {
                temp[count] = ownerAddr;
                count++;
            }
            unchecked { ++i; }
        }
